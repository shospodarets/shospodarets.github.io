---
layout: post
title: Finally Promise.prototype.finally() is available
tags: [JavaScript, Browsers]
share_image: https://static.hospodarets.com/img/blog/1505687007338770000.gif
share_description: Finally Promise.prototype.finally is available
draft: true
---
...
or even simpler case- when we just want to log the completion of the operation regardless of how it finished.
...
The idea and `finally` name came from the use cases and other libraries.
...
The clear need for such an API resulted in a proposal: [https://github.com/tc39/proposal-promise-finally](https://github.com/tc39/proposal-promise-finally)
Currently it's on Stage 3 of the TC39 process.

Availability of `Promise.prototype.finally()` brings it to all the Promise-based API, like global `fetch()` (used in the demos)
...
which we can use in `try\catch` blocks.
...
doesn't get any arguments (like the success data or reject error),
which is aligned with the API and purpose.
...
- Safari (version 11+)
- Chrome (63+ under the -​-enable-javascript-harmony flag) ([issue](https://bugs.chromium.org/p/v8/issues/detail?id=5967#c10))
...
You can integrate it in your project and use easily.
...
(it's based on [core-js](https://github.com/zloirock/core-js), which [includes Promise.prototype.finally](https://github.com/zloirock/core-js#stage-3-proposals))
